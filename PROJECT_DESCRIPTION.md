# TILEWARS Project Description

TILEWARS is a Devvit Web app built with Phaser 4 that runs inside Reddit Interactive Posts. It is a short logical deduction game played on a 5x5 grid. Each daily puzzle has one hidden connected pattern of 4-7 tiles, and the same puzzle is available to everyone for one UTC day.

Players reveal information by selecting tiles. Green identifies a pattern tile, while red, blue, and orange clues indicate pattern tiles in the same column, row, or diagonal. Some tiles combine multiple clues. Players use the information to narrow the board, mark possibilities with free X notes, and complete the pattern with as few guesses as possible. A completion reveal, personal result, and daily leaderboard make the outcome easy to understand and compare.
